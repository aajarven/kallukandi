...
Pelin lopputuloksen edullisuus pelaajalle määritellään palkintofunktiolla $R : S \rightarrow \mathbb{R}$. Usein käytetty palkintofunktio on arvoltaan $0$ pelitilanteissa jotka eivät ole lopputilanteita, ja lopputilanteille $1$ voittavista tilanteista, $0$ tasapeleistä ja $-1$ häviöistä.
...
Algoritmissa \ref{MCTS} esiteltyjen aliohjelmien \textsc{ValitseLapsi}, \textsc{Simuloi}, \textsc{Vastavirta} ja \textsc{ParasLapsi} toiminta on toteutusriippuvaista.
...
\textit{Upper Confidence Bounds for Trees}~\cite{browne, kocsis} (jatkossa UCT) on yleisimpiä algoritmeja MCTS:n toteutukseen. Laajentaessaan pelipuuta UCT pyrkii tasapainottelemaan hyväksi epäiltyjen pelitilanteiden lisätutkimuksen ja toistaiseksi vähän tutkittujen pelitilanteiden simuloinnin välillä.
...
Missä $s(v)$ on solmun $v$ kuvaama pelitilanne, $Q(v)$ on $v$:n kautta saavutetun palkinnon kokonaisarvo, $N(v)$ on $v$:n kautta kulkeneiden simulaatioiden lukumäärä. $\Delta(v, p)$ on pelaajan $p$ palkintoarvo solmussa $v$.
