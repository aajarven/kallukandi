...
Tehokkaan pelistrategian löytäminen monikätisen rosvon ongelmaan edellyttää algoritmia, joka osaa pelata paljon vivuilla joista on jo saatu hyviä tuloksia. Samalla kuitenkin sen on tutkittava välillä muitakin vipuja, jotta mahdollisesti huomaamatta jääneet paremmat vivut voidaan hyödyntää\footnote{Vastaavia valintatilanteita hyvien vaihtoehtojen käytön ja parempien valintojen etsimisen välillä kutsutaan tutkimus-hyväksikäyttö -dilemmaksi (\textit{exploration-exploitation dilemma}).}. Tällaisen strategian esittelevät Auer ym. joiden luottamusylärajan menetelmä~\cite{auer} (\textit{Upper Confidence Bounds}, jatkossa UCB1) valitsee pelattavaksi vivun joka maksimoi lausekkeen
\begin{equation}
