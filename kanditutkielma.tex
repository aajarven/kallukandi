...
Parhaat gon tekoälypelaajat käyttävät ensimmäisen siirron heuristiikkaa (\textit{All moves as first}, jatkossa AMAF)~\cite{browne}. AMAF päivittää laajennettavan solmun lisäksi simulaatiovaiheessa pelattavien siirtojen odotusarvoja $\Delta$:n mukaan. AMAF vaikuttaa simulaation tulokseen lisäämällä simulaatiokertojen määrää siirroille kasvattamatta tarvittaevien simulaatioiden määrää. Näin saadaan nopeasti tarkempi odotusarvo, joka voi kuitenkin olla harhaanjohtava, jos siirron arvo muuttuu paljon pelin edetessä.

AMAF:stä on useita muunnelmia, kuten $\alpha$-AMAF joka käyttää painokerrointa $\alpha$ painottaakseen solmun odotusarvon kertymistä. Kukin solmu saa odotusarvokseen
\begin{equation}
\alpha A + (1 - \alpha)U
\end{equation}
missä $A$ on solmun odotusarvo sen kautta simuloiduista peleistä ja $U$ on solmun odotusarvo siitä lähteneiden simulaatioiden perusteella. Erityisen suosittu AMAF-muunnelma on RAVE (\textit{Rapid Action Value Estimation}) joka toimii kuten $\alpha$-AMAF, mutta pienentää $\alpha$ arvoa solmun vierailukertojen mukaan. Tämä heuristiikka saa nopeasti karkeita arvioita, mutta vaihtaa hitaasti edetessään painotustaan kohti tarkempaa, tavallista UCT:tä~\cite{browne}.
